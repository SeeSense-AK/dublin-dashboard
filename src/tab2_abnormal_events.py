...
def render_tab2():
    ...
    if routes_added > 0:
        ...
        if 'tab2_map_render_key' not in st.session_state:
            st.session_state.tab2_map_render_key = 0
            st.session_state.tab2_map_render_key += 1
        map_data = st_folium(abnormal_map, height=500, width=None, key=f"abnormal_events_map_{st.session_state.tab2_map_render_key}")
        st.markdown('</div>', unsafe_allow_html=True)
