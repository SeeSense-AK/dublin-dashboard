...
def create_metric_summary(sensor_count, perception_count, corridor_count, sensor_top, perception_top, corridor_top):
    """Create professional metric summary using CSS classes"""
    col1, col2, col3 = st.columns(3)
    
    with col1:
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-label">Sensor Hotspots</div>
            <div class="kpi-value">{len(sensor_count)}</div>
            <div class="kpi-change">Displaying {len(sensor_top)}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-label">Perception Hotspots</div>
            <div class="kpi-value">{len(perception_count)}</div>
            <div class="kpi-change">Displaying {len(perception_top)}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-label">Corridor Hotspots</div>
            <div class="kpi-value">{len(corridor_count)}</div>
            <div class="kpi-change">Displaying {len(corridor_top)}</div>
        </div>
        """, unsafe_allow_html=True)

def create_hotspot_card_enhanced(row, source):
    """Enhanced professional hotspot card display using Streamlit columns"""
    ...
    # Determine card style based on urgency/priority
    urgency_value = float(urgency_score.replace('%', ''))
    if source == 'corridor':
        card_class = f"hotspot-card hotspot-card-{priority_category.lower()}"
    else:
        if urgency_value >= 70:
            card_class = "hotspot-card hotspot-card-critical"
        elif urgency_value >= 50:
            card_class = "hotspot-card hotspot-card-high"
        else:
            card_class = "hotspot-card hotspot-card-medium"
    ...
    # Source badge
    source_badge = {
        'sensor': 'Core Sensor Data',
        'perception': 'Perception + Sensor', 
        'corridor': 'Corridor Reports'
    ...
    with st.container():
        ...
        <div class="{card_class}">
            <div class="hotspot-header">
                <h3 class="hotspot-title">{hotspot_name}</h3>
                <span class="source-badge">{source_badge}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        # Use Streamlit columns for the grid layout (more reliable than CSS grid)
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.markdown("""
            <div class="hotspot-grid-item">
                <div class="hotspot-label">Location</div>
                <div class="hotspot-value">""" + location + """</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown("""
            <div class="hotspot-grid-item">
                <div class="hotspot-label">Event Type</div>
                <div class="hotspot-value">""" + event_type + """</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            st.markdown("""
            <div class="hotspot-grid-item">
                <div class="hotspot-label">Unique Devices</div>
                <div class="hotspot-value">""" + str(device_count) + """</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            st.markdown("""
            <div class="hotspot-grid-item">
                <div class="hotspot-label">User Reports</div>
                <div class="hotspot-value">""" + str(user_reports) + """</div>
            </div>
            """, unsafe_allow_html=True)
        
        # Urgency score section
        st.markdown(f"""
        <div class="hotspot-urgency">
            <div class="hotspot-label">Urgency Score</div>
            <div class="urgency-score {urgency_class}">{urgency_score}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            view_details = st.button("View Detailed Analysis", key=f"details_{hotspot_name}", width="stretch")
        
        with col2:
            street_view_url = STREET_VIEW_URL_TEMPLATE.format(lat=lat, lng=lng, heading=0)
            st.link_button("Open Street View", street_view_url, width="stretch")
        
        # Show details if button clicked
        if view_details:
            display_hotspot_details_enhanced(row, source)
        
        st.markdown("---")

def display_hotspot_details_enhanced(row, source):
    """Enhanced display of detailed AI analysis using CSS classes"""
    ...
    with st.spinner("Generating AI insights..."):
        ...
        # Use your original AI functions
        try:
        ...
        # Enhanced display with CSS classes
        st.markdown("""
        <div class="analysis-section">
            <h4 class="analysis-title">AI Analysis Summary</h4>
        </div>
        """, unsafe_allow_html=True)
        st.info(insights['summary'])
        
        # Display themes as professional pills
        if insights['themes']:
            st.markdown("""
            <div class="analysis-section">
                <h4 class="analysis-title">Key Safety Themes</h4>
            </div>
            """, unsafe_allow_html=True)
            ...
            for idx, theme in enumerate(insights['themes']):
                with theme_cols[idx]:
                    st.markdown(
                        f'<div class="theme-pill">{theme}</div>',
                        unsafe_allow_html=True
                    )
        ...
        # Enhanced user comments display
        if user_comments and len(user_comments) > 0:
            st.markdown("""
            <div class="analysis-section">
                <h4 class="analysis-title">User Feedback</h4>
            </div>
            """, unsafe_allow_html=True)
            for i, comment in enumerate(user_comments[:3], 1):
                st.markdown(
                    f'<div class="user-comment"><strong>Comment {i}:</strong> {comment}</div>',
                    unsafe_allow_html=True
                )

# ────────────────────────────────────────────────
# ORIGINAL FUNCTIONS (COPIED FROM YOUR tab1_hotspots.py)
# ────────────────────────────────────────────────
...
def render_tab1_enhanced():
    ...
    create_section_header("Hotspot Analysis", "Analysis combining sensor data, perception reports, and corridor surveys")
    
    # Load data using your original function
    try:
    ...
    # Display data distribution info in enhanced format
    ...
    # Enhanced metric summary
    create_metric_summary(sensor_df, perception_df, corridor_df, sensor_top, perception_top, corridor_top)
    ...
    # Enhanced hotspot details section
    st.markdown("---")
    create_section_header("Hotspot Details", "Detailed analysis of identified safety concerns")
    ...
    for row, source in all_hotspots:
        create_hotspot_card_enhanced(row, source)
